...
In order to facilitate debugging during maintenance and extension of Pod-Html,
we want to be able to execute our programs either via the core distribution's
test harness:

    cd t; ./perl harness ../ext/Pod-Html/t/*.t; cd -

... or, for an individual program, directly from the top-level of the core
distribution (possibly through the debugger):

    ./perl -Ilib -d ext/Pod-Html/t/htmldir1.t

To provide us with this flexibility, our test program has to know how to
populate C<@INC> correctly for each case.  This is done in a C<BEGIN> block at
the top of the program.  This then enables us to C<use> modules like
F<Test::More>, F<Cwd> and this module correctly and to record what current
working directory we're starting out from and will ultimately have to return
to.
...
actually located in a directory two levels down whose relative path is
...
F<perl>.) Note that this means that at the end of the program you will have to
...
The hashref is then passed to C<xconvert()> which internally generates an expected HTML output string by massaging the text read in from the C<DATA> handle.   C<xconvert()> reads in the relevant F<t/*.pod> file and passes it to C<Pod::Html::pod2html()>, which parses the POD and generates the actual HTML output.  If "got" matches "expected", a PASS is recorded for this instance of C<xconvert()>.
...
Note that in the two "runs" above, the values for C<podstub> are C<expect> the
same, but the arguments to C<p2h> differ and we've distinguished the two runs
by different values for C<description>.

=back

Note that all runs within a given F<t/*.t> program share the same temporary directory.  Since
C<Pod::Html::pod2html()> typically caches its understanding of where F<.pod> files are located, there is a possibility that the contents of the cache may affect the generated HTML output in an adverse way.  This possibility will be addressed in an upcoming version of this program.

When all runs have been completed (as noted above), we change back to the directory we started from to permit the temporary directory and its contents to be cleanly deleted.
...
The current working directory as determined by a call to F<cwd> at the start
of your program.  Required.
...
sub xconvert {
    ...
    for my $k ('podstub', 'description', 'expect') {
        die("xconvert() must have $k element")
